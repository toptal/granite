RSpec.describe 'satisfy_preconditions', aggregate_failures: false do # rubocop:disable RSpec/DescribeClass
  before do
  ...
  context 'with failing preconditions' do
    ...
    specify do
      expect(action).not_to satisfy_preconditions.with_messages('Precondition failed', /failed$/, /^Precondition/)
    end

    specify do
      expect do
        expect(action).to satisfy_preconditions
      end.to fail_with(%(expected #{action} to satisfy preconditions but got following errors:\n ["Precondition failed"])) # rubocop:disable Layout/LineLength
    end
