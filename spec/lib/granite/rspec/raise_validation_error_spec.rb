RSpec.describe 'raise_validation_error', aggregate_failures: false do # rubocop:disable RSpec/DescribeClass
  ...
  context 'when action raises error' do
    ...
    specify do
      expected_errors = { base: [{ error: :some_error }] }.inspect

      expect do
        expect { action.perform! }.to raise_validation_error.of_type(:some_error2)
      end.to fail_with("expected to raise validation error on attribute :base of type :some_error2, but raised #{expected_errors}") # rubocop:disable Layout/LineLength
    end
    ...
    specify do
      ...
      end.to fail_with(include "expected to raise validation error on attribute :raise_error, but raised #{expected_errors}") # rubocop:disable Layout/LineLength
    end
    ...
    specify do
      ...
      end.to fail_with(include "expected to raise validation error on attribute :raise_error of type :some_error, but raised #{expected_errors}") # rubocop:disable Layout/LineLength
    end
  end
end
